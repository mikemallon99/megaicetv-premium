...
window.fetch = async function(...args) {
	...
	if (!contentType.includes('application/json')) {
        ...
        if (data && data.contents?.twoColumnWatchNextResults?.secondaryResults?.secondaryResults?.results?.[1]?.itemSectionRenderer?.contents) {
	    data.contents.twoColumnWatchNextResults.secondaryResults.secondaryResults.results[1].itemSectionRenderer.contents.forEach(video => {
        ...
        if (data && data.contents?.twoColumnBrowseResultsRenderer?.tabs) {
